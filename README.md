...
TRELLIS.2 · FlexGEMM · CuMesh 는 **커밋이 고정되어 있다**
(`setup_trellis2.sh` 상단의 `*_COMMIT`).

`apply_patches.py` 가 문자열 치환으로 패치하므로, 업스트림이 해당 파일을
한 줄만 고쳐도 패치가 실패한다. 커밋을 열어두면 설치 날짜에 따라 다른 환경이
만들어진다. 업스트림을 따라갈 때는 커밋을 바꾸고 **전체 설치를 다시 검증**할 것.
