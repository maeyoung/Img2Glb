...
def _build_parser():
    ...
    pv.add_argument("--output", "-o", type=Path, default=None,
                    help="출력 PNG (기본: output/render/<이름>_preview.png)")
...
def _default_output(image: Path) -> Path:
    return Path.cwd() / "output" / "glb" / f"{image.stem}.glb"


def _default_render(glb: Path) -> Path:
    return Path.cwd() / "output" / "render" / f"{glb.stem}_preview.png"
...
def main(argv=None):
    ...
    if args.preview:
        from .tools.preview import render
        render(result, _default_render(result))
    return 0
